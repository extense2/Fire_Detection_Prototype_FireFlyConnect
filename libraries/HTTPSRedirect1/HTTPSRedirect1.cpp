...
HTTPSRedirect1::HTTPSRedirect1(const int p, const char* fp, bool c) 
    : httpsPort(p), redirFingerprint(fp), fpCheck(c){
}

HTTPSRedirect1::HTTPSRedirect1(const int p) 
    : httpsPort(p){
      fpCheck = false;
}

HTTPSRedirect1::~HTTPSRedirect1(){ 
}

bool HTTPSRedirect1::printRedir(String& url, const char* host, const char* redirHost){
  return printRedir(url.c_str(), host, redirHost);
}

bool HTTPSRedirect1::printRedir(const char* url, const char* host, const char* redirHost){
...
String HTTPSRedirect1::createRequest(const char* url, const char* host){
...
void HTTPSRedirect1::fetchData(bool disp, bool header){
  String line;
  while (connected()) {
    line = readStringUntil('\n');
